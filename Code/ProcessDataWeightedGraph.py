...
def main(argv):
	...
	try:
		...
		print("ProcessDataWeightedGraph.py -d <dataset> -D <database> -m <model:0(import)/1(clean)> -e <edge:0(undirected)/1(directed)>")
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print("ProcessDataWeightedGraph.py -d <dataset> -D <database> -m <model:0(import)/1(clean)> -e <edge:0(undirected)/1(directed)>")
	...
	if model == 0:
		...
		edge_file = open(cur_path + "../Data/Graph/" + dataset + edge_file_path, 'r')
		node_file = open(cur_path + "../Data/Graph/" + dataset + "/node.txt", 'r')

		code = "CREATE TABLE node (ID INTEGER NOT NULL);"
		cur.execute(code)
		if edge == 1:
			code = "CREATE TABLE edge (WEIGHT DECIMAL(15,2) NOT NULL, FROM_ID INTEGER NOT NULL, TO_ID INTEGER NOT NULL);"
		else:
