...
def ReadInput():
	...
	obj = np.append(np.zeros(num_users), np.ones(num_counts))
	ub = np.ones(num_users + num_counts)
...
def main(argv):
	...
	try:
		...
		print("ComputeR_Distinct.py -I <input path> -e <epsilon> -b <beta> -D <upper bound> -d <error level> -O <output path> -p <processor num>")
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print("ComputeR_Distinct.py -I <input path> -e <epsilon> -b <beta> -D <upper bound> -d <error level> -O <output path> -p <processor num>")
