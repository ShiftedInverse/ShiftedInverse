	...
	for i in range(len(queries_Sj)):
		for j in range(len(scales)):
			...
			for k in range(repeat_times_Sj):
				...
				cmd = "../python/python " + cur_path + "/../Code/ComputeR_Sum" + folders_Sj[i] + ".py -I ../Information/TPCH/" + scales[j] + "/" + queries_Sj[i] + "_count.txt -e " + str(epsilon / 2) + " -b 0.1 -D " + str(int(upper_bounds_count_Sj[i] * math.pow(2, j))) + " -d " + str(int(error_levels_count_Sj[i])) + " -O ../Temp/TPCH" + scales[j] + "_Sum" + folders_Sj[i] + "_" + queries_Sj[i] + "_count_" + str(epsilon / 2) + ".txt -p " + str(p2)

				shell = os.popen(cmd, 'r')
				shell.read()
				shell.close()

				cmd = "../python/python " + cur_path + "/../Code/Exponential.py -I ../Temp/TPCH" + scales[j] + "_Sum" + folders_Sj[i] + "_" + queries_Sj[i] + "_count_" + str(epsilon / 2) + ".txt -e " + str(epsilon / 2) + " -b 0.1 -D " + str(int(upper_bounds_count_Sj[i] * math.pow(2, j))) + " -d " + str(int(error_levels_count_Sj[i]))
				...
				cmd = "../python/python " + cur_path + "/../Code/ComputeR_Selection" + folders_Sj[i] + ".py -I ../Temp/TPCH" + scales[j] + "_" + queries_Sj[i] + "_median.txt -e " + str(epsilon / 2) + " -b 0.1 -D " + str(upper_bounds_Sj[i] - lower_bounds_Sj[i]) + " -d " + str(error_levels_Sj[i]) + " -O ../Exponential/TPCH" + scales[j] + "_Selection" + folders_Sj[i] + "_" + queries_Sj[i] + "_" + str(epsilon) + "_median.txt -k " + str(index) + " -p " + str(p2)

				shell = os.popen(cmd, 'r')
				shell.read()
				shell.close()

				cmd = "../python/python " + cur_path + "/../Code/Exponential.py -I ../Exponential/TPCH" + scales[j] + "_Selection" + folders_Sj[i] + "_" + queries_Sj[i] + "_" + str(epsilon) + "_median.txt -e " + str(epsilon) + " -b 0.1 -D " + str(upper_bounds_Sj[i]) + " -d " + str(error_levels_Sj[i])
				...
				error, percentile = GetRankError_Sj(i, scales[j], output, percent)

				output_file.write(str(output) + " " + str(error) + " " + str(percentile) + " " + str(abs(output - answer)) + "\n")
		
				errors_Sj.append(abs(output - answer))
...
def main(argv):
	...
	cur_path = os.getcwd()
	output_file = open(cur_path + "/../Result/TPCH/ResultMedian.txt", 'w')
