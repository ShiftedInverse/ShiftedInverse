...
queries = ["Q_t", "Q_op"]
relations = ["sc", "sc"]
upper_bounds = [6000000, 6000000]
scales = ["_3"]
epsilons = [1]

def ReadInput(j, i):
	global user_value_dict
	global value_list
	global query_result
	...
	for line in input_file.readlines():
		...
		tuple_value = str(elements[0])
		user_id = int(elements[1])

		if tuple_value in value_dict.keys():
			value_dict[tuple_value] += 1
		else:
			value_dict[tuple_value] = 1

		if user_id in user_value_dict.keys():
			if tuple_value in user_value_dict[user_id].keys():
				user_value_dict[user_id][tuple_value] += 1
			else:
				user_value_dict[user_id][tuple_value] = 1
		else:
			user_value_dict[user_id] = {}
			user_value_dict[user_id][tuple_value] = 1 

	for user_id in user_value_dict.keys():
		user_dict[user_id] = max(user_value_dict[user_id].values())

	value_list = sorted(user_dict.values())
	query_result = max(value_dict.values())
		...
		for j in range(len(scales)):
			for epsilon in epsilons:
				...
				for k in range(repeat_times):
					...
					for user_id in user_value_dict.keys():
						for tuple_value in user_value_dict[user_id].keys():
							if tuple_value in value_dict_2.keys():
								value_dict_2[tuple_value] += min(bound, int(float(user_value_dict[user_id][tuple_value])))
							else:
								value_dict_2[tuple_value] = min(bound, int(float(user_value_dict[user_id][tuple_value])))

					ans = max(value_dict_2.values())
...
def main(argv):
	...
	cur_path = os.getcwd()
	output_file = open(cur_path + "/../Result/TPCH/ResultFrequencyZetaSql.txt", 'w')
